...
import { QUERY_GET, QUERY_GET_ALL, QUERY_CREATE, QUERY_UPDATE, QUERY_DELETE } from "../database/tables/categories.js";
...
router.post("/", authenticator, (req, res, next) => {
  const paramList = [req.body.name, req.body.icon, req.userId];
  queryHandler(QUERY_CREATE, paramList, res, next);
});

router.put("/:id", authenticator, (req, res, next) => {
  const paramList = [req.body.name, req.body.icon, req.params.id];
